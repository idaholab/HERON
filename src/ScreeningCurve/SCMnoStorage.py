...
def Storage(netGen,gen_solar,gen_wind,C,E,HC,HD): #function to determine CAES system size, charging/discharging hours, cost
    ...
    compressorSize = (Cap/charging) #determines compressor size [MW]
    compressorSize = compressorSize.astype(np.float)
    #print("Compressor size is type ",type(compressorSize))
    turbineSize = (Cap/HD) #determins turbine size [MW]
    # Demand2=pd.DataFrame()
    # Demand2["Demand (MW)"] = netGen
    ...
    for j in range(len(f)): #loop through every day of the year
        d = j*24
        day = Demand2[d:d+24] #isolates the hours within the day
        days = pd.DataFrame(day)
        #print(days)
        
        dayR = days.reset_index() #resets index and creates column "index" representing the hour of the year
        #print("dayR")
        #print(dayR)
        ...
        daySort=day.sort_values(by=['Demand (MW)'])
        daySort = daySort.reset_index()
        #print("daySort")
        #print(daySort)
        dayMax = day.sort_values(by=['Demand (MW)'], ascending=False) #sorts values from high to low, used for discharging
        dayMax = dayMax.reset_index()
        #print("dayMax")
        #print(dayMax)
        Cap2 = Cap #sets capacity of the storage system
        ...
        for i in range(hrChrUp): #loop through the charging periods
          for j in range(len(dayR)): #loop through the whole day
             if dayR.loc[j]['Demand (MW)']==daySort.loc[i]['Demand (MW)']: #matches the daily demand to the periods of lowest demand to perform charging 
             ...
             if Cap2<=0: #if we have filled up our storage space, we move on to discharging 
                 for k in range(math.ceil(HD)): #loop through discharge periods
    ...
    storageCost = 105.6107158535297*1000*Cap + 3.00*(charging*compressorSize + HD*turbineSize)*len(f)   #105 is ~annualized fixed cost, 3.00 is VOM, the sum of the hours is daily hourly use. Comp*Eff is turbine size
    # netGen = Demand2
...
def evaluate_cost(LL,self,taxRate): #Determines cost curve at each MW of generation
        self["cost"] = self["FOM"] + self["VOM"] * (LL['counts']-1) + self["CarbonEmission"] * (LL['counts']-1) *taxRate
...
def load_levels(netGen,resolution, maxLoad): #Determines number of steps to use for given resolution, 1 MW should ALWAYS be used. If this is changed, scale all costs accordingly
    ...
    loadCurve = netGen
    loadLevel = np.linspace(0,maxLoad, num=steps+1) #loadLevel is set from 0-maximum load, with step size equivalent to 1 MW
    
    return loadCurve, loadLevel, steps

def counts(loadLevel, loadCurve): #Counts number of operational hours at each load level
    LL = pd.DataFrame(data=loadLevel)
...
def run2(netGen, TR, *kwargs):
    ...
    maxLoad = max(load)
    taxRate = TR                                                    #$/ton CO2
    loadCurve, loadLevel, steps = load_levels(netGen,1,maxLoad)
...
def run(raven,Input):
  ...
  print(len(netGen))
####  
  #netGen, storageCost, compressorSize, turbineSize, capacityStorage = Storage(netGen,gen_solar,gen_wind,raven.C,0.5,raven.HC,raven.HD)
  #print("Storage ran")
...
#####
  ...
  #print("Capacity of storage is ",capacityStorage)
