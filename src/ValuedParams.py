...
class ValuedParam:
  ...
  def get_input_specs(cls, name):
    ...
    """
    str_name=name
    desc=r"""The node \xmlNode{"""+str_name+r"""} has the following \textcolor{red}{\textit{ValuedParams}} options:"""
    spec = InputData.parameterInputFactory(name,descr=desc)
    # for when the value is fixed (takes precedence over "sweep" and "opt") ...
    spec.addSub(InputData.parameterInputFactory('fixed_value', contentType=InputTypes.FloatType,descr=r"""It can be a fixed value."""))
    # for when the value is parametric (only in "sweep" mode)
    spec.addSub(InputData.parameterInputFactory('sweep_values', contentType=InputTypes.FloatListType,descr=r"""It can be a value which is to be swept over multiple values."""))
    # for when the value is optimized (only in "min" or "max" mode)
    spec.addSub(InputData.parameterInputFactory('opt_bounds', contentType=InputTypes.FloatListType, descr=r"""Opt bounds."""))
    # for when the value is time-dependent and given by an ARMA
    arma = InputData.parameterInputFactory('ARMA', contentType=InputTypes.StringType, descr=r"""It can be an \textbf{A}uto \textbf{R}egressive \textbf{M}oving \textbf{A}verage value.""")
    arma.addParam('variable', param_type=InputTypes.StringType, descr=r"""Variable generated by an ARMA model.""")
    spec.addSub(arma)
    # for when the value comes from evaluating a function
    func = InputData.parameterInputFactory('Function', contentType=InputTypes.StringType, descr=r"""It can be a value generated by running a function.""")
    func.addParam('method', param_type=InputTypes.StringType,descr=r"""The method containing the function.""")
    spec.addSub(func)
    # for when the value comes from another variable
    var = InputData.parameterInputFactory('variable', contentType=InputTypes.StringType,descr=r"""Variable""")
    spec.addSub(var)
    # for when the result obtained needs to grow from year to year
    growth = InputData.parameterInputFactory('growth', contentType=InputTypes.FloatType,descr=r"""Growth factor required to grow the variable from one year to another.""")
    growth_mode = InputTypes.makeEnumType('growthType', 'growthType', ['linear', 'exponential'])
    growth.addParam('mode', param_type=growth_mode, descr=r"""The growth mode can be linear or exponential.""")
    spec.addSub(growth)
    return spec
