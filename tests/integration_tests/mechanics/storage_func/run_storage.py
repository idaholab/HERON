...
def tiered(data, meta):
  ...
  level = comp.get_interaction().get_initial_level(meta)
  activity = np.zeros(len(time))
  for t, _ in enumerate(time):
    if price[t] > 0.7:
      # empty storage and sell it all, up to generator capacity
      release = min(level, gen_cap - steam_produced)
      level -= release
    elif price[t] < 0.3:
      # take and store all the electricity we can
      absorb = min(steam_produced, capacity - level)
      level += absorb
    activity[t] = level
